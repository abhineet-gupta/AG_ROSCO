...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('\n')
    file.write('!--------------------------------------------------------------------------------------------------------------\n')
    file.write('!------- CONTROLLER FLAGS - Everything below here is unused for QED controller --------------------------------\n')
    ...
    file.write('{0:<12d}        ! Y_ControlMode		- Yaw control mode {{0: no yaw control, 1: yaw rate control, 2: yaw-by-IPC, 3: yaw for speed regulation}}\n'.format(int(rosco_vt['Y_ControlMode'])))
